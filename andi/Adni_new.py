...
class Adni(object):
    ...
    def check(self):
        assert (self.k <= self.b), "diffusion users should be less than group size"
        assert (self.k <= self.u), "diffusion users should be less than users in total"
        assert (self.c2 >= 2 * self.c4), "condition 2 violated"
        assert (self.c6 >= 2 * self.c5), "condition 3 violated"
        assert (self.c3 >= 8 * self.c5), "condition 4 violated"
        assert (self.c4 >= 4 * self.c5), "condition 5 violated"
        assert ((0.5 / self.c6 - 1.0 / self.c3 - 0.5 / self.c5 / self.c6) >= 1.0 / self.c4), "condition 6 violated"
        assert (0.5 / self.c5 >= (1.2 / self.c6  + 1.0 / self.c1)), "condition 7 violated"
        assert (0.2 >= (1.0  / self.c5 + (4.0 * self.c6) / (3.0 * self.c3) + 0.5 / self.c1 + 0.5 / self.c2)), "condition 8 violated"
    ...
    def diffusion(self, X, y):
        ...
        while(t <= self.tlast):
            ...
            if((np.nonzero(s.indptr[1:] - s.indptr[:-1])[0] < self.u).sum() < self.k):
                t += 1
                continue
            ...
            ## check the condition
            qualify = False
            users = (indices[:j] < self.u).sum()
            while(j < s.getnnz()):
                sum_of_degree += ordered_degree[j][0]
                #calculate the potential users
                users += indices[j] < self.u
                # condition 1
...
if __name__ == "__main__":
    ...
    y = np.array([2,3])
    u = 2 # number of user node
    k = 2 # propagate size
    b = 2 # group size
    adni = Adni(b=b,k=k,u=u)
